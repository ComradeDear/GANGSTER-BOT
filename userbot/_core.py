...
from userbot import *
from userbot import bot as gangsterbot

DELETE_TIMEOUT = 5
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "GANGSTER User"
gangster_logo = "./H1M4N5HU0P/mafiabot_logo.jpg"
h1m4n5hu0p = mafiabot.uid
gangster = f"[{DEFAULTUSER}](tg://user?id={h1m4n5hu0p})"

@gangsterbot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@gangsterbot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
...
@gangsterbot.on(admin_cmd(pattern="install$", outgoing=True))
@gangsterbot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {mafia}\n\n{string}\n\n        ⚡ **[GANGSTER-BOT](t.me/GangsterBot_Support)** ⚡", link_preview=False)
...
@gangsterbot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@gangsterbot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@gangsterbot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@gangsterbot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@gangsterbot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@gangsterbot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
