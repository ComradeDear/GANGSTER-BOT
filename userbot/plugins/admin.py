...
async def promote(promt):
    ...
    gangsterevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await gangsterevent.edit("Promoted Successfully!")
    except BadRequestError:
        await gangsterevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    gangsterevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await gangsterevent.edit(NO_PERM)
        return
    await gangsterevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    gangsterevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await gangsterevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await gangsterevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await gangsterevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    gangsterevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await gangsterevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await gangsterevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    gangsterevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await gangsterevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        await gangsterevent.edit(
            f"Kicked [{user.first_name}](tg://user?id={user.id})!\nReason: {reason}"
        )
    else:
        await gangsterevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
